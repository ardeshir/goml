...
func main() {
	...
	versicolorDF = irisDF.Filter(filter).Select([]string{"sepalen", "species"})
	fmt.Println(versicolorDF)

	// filter the dataframe again, but only display
	// first three results
    versicolorDF = irisDF.Filter(filter).Select([]string{"sepalen", "species"}).Subset([]int{0,1,2})
